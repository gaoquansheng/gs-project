...
import {
  ...
  exportUnitreview,
  getProjectStatus,
} from "@/api/pm/unitreview";
import {
  getProjectList,
  getProjectTask,
  getProjectReport,
} from "@/api/pm/projectList";
  ...
  methods: {
    ...
    getList() {
      ...
      listUnitreview(this.queryParams).then((response) => {
        this.projectreviewList = response.rows;
    ...
    handleReject(row) {
      let tmp = {};
      tmp.projectId = row.projectId;
      tmp.projectStatus = -row.projectStatus;
      updateUnitreview(tmp).then((res) => {
        if (res.code == 200) {
          this.msgSuccess(res.msg);
          this.getList();
    ...
    handleView(row) {
      // console.log(row);
      if (this.queryParams.projectStatus.startsWith("1")) {
        getProjectList(row.projectId).then((response) => {
          window.open(response.data.pdfPath);
        });
      }
      if (this.queryParams.projectStatus.startsWith("2")) {
        getProjectTask(row.projectId).then(response => {
          window.open(response.data.pdfPath)
        })
      }
      if (this.queryParams.projectStatus.startsWith("4")) {
        getProjectReport(row.projectId).then(response => {
          window.open(response.data.pdfPath)
        })
      }
