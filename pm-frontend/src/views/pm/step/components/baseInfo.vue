  ...
  <div>
    <el-form label-width="115px" ref="form" :model="form" :rules="rules">
      <el-row :gutter="10">
        ...
        <el-col :span="6">
          <el-form-item label="业务主管部门" prop="proxyOrgCode">
            <treeselect
              v-model="form.proxyOrgCode"
              :options="deptOptions"
        ...
        <el-col :span="6">
          <el-form-item label="投资来源" prop="moneyFrom">
            <el-select v-model="form.moneyFrom" multiple placeholder="请选择项目投资来源">
              <el-option
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item label="总投资(万)">
            <el-input v-model.number="form.totalAmount" type="number"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="中央投资(万)">
            <el-input v-model.number="form.centralAmount"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="地方投资(万)">
            <el-input v-model.number="form.localAmount"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="自筹投资(万)">
            <el-input v-model.number="form.myAmount"></el-input>
          </el-form-item>
        </el-col>                
      </el-row>
      <el-row>
        <el-col >
          <el-form-item label="建设目标" prop="keyWords">
            <el-input
              type="textarea"
              :rows="3"
...
import { addProject, updateProject, getProjectList } from "@/api/pm/step.js";
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if(valid){
          ...
          this.form.projectStatus = this.params.projectStatus;
          //因为这里投资来源是个列表，所以需要处理一下
          this.form.moneyFrom = this.form.moneyFrom.join(",");
          if (!this.params.projectId) {
            addProject(this.form).then((response) => {
              if (response.code === 200) {
                this.params.projectId = response.projectId;
                this.$emit("update:initParams", this.params);
    ...
    getForm() {
      if (this.params.projectId) {
        ...
        getProjectList(this.params.projectId).then((response) => {
          ...
          //投资来源多选回显
          this.form.moneyFrom = this.form.moneyFrom.split(",");
        });
      }
    },
    getTreeselect() {
      treeselect().then((response) => {
        ...
        this.deptOptions = response.data[0].children.filter(child => {
          return child.label == "机关"
        })
      });
    },
    reset_form() {
